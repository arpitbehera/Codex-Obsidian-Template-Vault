...
This file is the **single source of truth** for all active agents in the crew. The active runtime wrapper and all agents reference this file for routing decisions and inter-agent coordination.
...
1. The active runtime wrapper reads the `Input` column to match user messages to agents
2. The active runtime wrapper reads `Output` and `Capabilities` to decide if chaining is needed after an agent returns
...
3. The Architect generates the agent file in the active custom-agent location, adds a row to the Registry table above, and updates `agents.md`
4. The active runtime wrapper makes the new agent available according to its own loading rules
...
Custom agents always have lower routing priority than the 8 core agents. The active runtime wrapper checks custom agents only when no core agent matches the user's message. Among custom agents, it uses the Input column to find the best match

---

## Skills Registry

Skills handle complex, multi-step workflows extracted from agents. They are checked **before** agents by the active runtime wrapper. The wrapper may enter them through natural language, slash commands, or direct skill selection.
...
1. The active runtime wrapper checks the skill routing rules before the agent routing rules
2. If a trigger matches, enter the skill flow instead of dispatching the source agent directly
3. If no skill matches, fall through to agent routing
4. Skills can produce `### Suggested next agent` output, which the active runtime wrapper handles using the same chaining rules as agents
