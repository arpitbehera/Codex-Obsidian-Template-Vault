...
For the definitive list of agents with capabilities, inputs, outputs, and status, see `references/agents-registry.md` in the repo or the equivalent runtime copy. That file is the single source of truth for both core and custom agents.
...
**Skills**: Complex flows (onboarding, defrag, agent creation or management) are handled by dedicated skills. A runtime may enter them via slash command, natural language, or direct operator choice.
...
**Skills**: Full audit, deep clean, and tag garden are handled by dedicated skills.
...
**Skills**: Most transcription processing is handled by the transcribe skill. The agent handles only edge cases.
...
**Skills**: Email triage, meeting prep, weekly agenda, and deadline radar are handled by dedicated skills.
...
Skills handle complex, multi-step workflows. The active runtime may enter them through natural language, slash commands, or direct skill selection.

The active runtime wrapper should route triggers to skills first, then fall through to agents.
...
When an agent detects work for another agent, it includes a `### Suggested next agent` section in its output. The active runtime wrapper decides whether to continue the chain. See `references/agent-orchestration.md` for the full protocol.
...
Custom agents are created by the Architect and live either in `agents/custom/` for repo-local Codex use or in `.claude/agents/` for Claude vault installs. They follow the same conventions: metadata frontmatter when needed, trigger phrases written in the user's language, and the shared orchestration protocol.

For the definitive list of all agents (core + custom) with capabilities, inputs, outputs, and status, see `references/agents-registry.md` in the repo or the equivalent runtime copy.
...
Say "create a new agent" or "I need a custom agent" to start the process. The create-agent skill guides you through a structured interview to define the agent's purpose, triggers, permissions, and coordination rules.
