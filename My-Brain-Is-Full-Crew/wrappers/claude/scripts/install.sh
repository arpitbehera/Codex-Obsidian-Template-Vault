...
if [[ -f "$REPO_DIR/wrappers/claude/settings.json" ]]; then
  if [[ -f "$VAULT_DIR/.claude/settings.json" ]]; then
    ...
    cp "$REPO_DIR/wrappers/claude/settings.json" "$VAULT_DIR/.claude/settings.json"
...
if [[ "$MCP_ANSWER" =~ ^[Yy]$ ]]; then
  if [[ -f "$VAULT_DIR/.mcp.json" ]]; then
    ...
    cp "$REPO_DIR/wrappers/claude/mcp.sample.json" "$VAULT_DIR/.mcp.json"
...
echo ""
echo -e "   ${DIM}To update after a git pull: bash wrappers/claude/scripts/update.sh${NC}"
echo ""
