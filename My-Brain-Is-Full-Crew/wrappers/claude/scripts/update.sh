...
# ── Update hooks ──────────────────────────────────────────────────────────
HOOK_COUNT=0
if [[ -d "$REPO_DIR/wrappers/claude/hooks" ]]; then
  mkdir -p "$VAULT_DIR/.claude/hooks"
  for hook in "$REPO_DIR/wrappers/claude/hooks/"*.sh; do
    [[ -f "$hook" ]] || continue
    name="$(basename "$hook")"
...
SETTINGS_UPDATED=""
if [[ -f "$REPO_DIR/wrappers/claude/settings.json" ]]; then
  dst="$VAULT_DIR/.claude/settings.json"
  if [[ ! -f "$dst" ]] || ! diff -q "$REPO_DIR/wrappers/claude/settings.json" "$dst" >/dev/null 2>&1; then
    mkdir -p "$VAULT_DIR/.claude"
    cp "$REPO_DIR/wrappers/claude/settings.json" "$dst"
...
# ── Update CLAUDE.md ──────────────────────────────────────────────────────
CLAUDE_MD_UPDATED=""
if [[ -f "$REPO_DIR/wrappers/claude/CLAUDE.md" ]]; then
  if [[ ! -f "$VAULT_DIR/CLAUDE.md" ]] || ! diff -q "$REPO_DIR/wrappers/claude/CLAUDE.md" "$VAULT_DIR/CLAUDE.md" >/dev/null 2>&1; then
    cp "$REPO_DIR/wrappers/claude/CLAUDE.md" "$VAULT_DIR/CLAUDE.md"
