...
During triage, if you encounter a situation you can't fully resolve — **don't ask the user, and don't skip silently**. Signal the active runtime wrapper or operator via your output.
...
- **Ambiguous destination**: if you have 2-3 reasonable options, ask the user one direct clarification question. If the vault is missing the right area entirely, leave a follow-up suggestion for the Architect and file provisionally in the best available location
