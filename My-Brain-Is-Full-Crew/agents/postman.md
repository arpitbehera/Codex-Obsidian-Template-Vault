...
If `gws` is not installed or not authenticated, fall back to the MCP tools defined in the vault-level `.mcp.json` when that runtime file exists:
...
MCP tools **cannot** archive, delete, label, mark as read, send emails, or modify/delete calendar events. If the user requests a write operation and only MCP is available, inform them that `gws` is required and point them to `../docs/gws-setup-guide.md`.
...
> Requires `gmail.send` scope in addition to `gmail.modify`. See `../docs/gws-setup-guide.md`.
